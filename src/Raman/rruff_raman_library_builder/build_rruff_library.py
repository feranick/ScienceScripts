#!/usr/bin/env python3
"""
build_rruff_library.py
======================

Downloads the RRUFF Raman reference spectra (zipped archives) and packs them
into a single consolidated HDF5 "library" file that the Raman plotter tools can
open directly. Peak positions are pre-computed and stored per spectrum, so the
"Match by Selected Peaks" feature becomes near-instant (no re-parsing / re-detecting
thousands of text files each time).

Run it once in a while to refresh the library:

    python build_rruff_library.py                       # all sets -> rruff_raman_library.h5
    python build_rruff_library.py --datasets excellent_unoriented
    python build_rruff_library.py --out my_lib.h5 --refresh

Downloaded zip archives are cached (default ./rruff_cache) so re-runs only fetch
what changed unless --refresh is given.

Source: https://rruff.info/  (Lafuente B, Downs R T, Yang H, Stone N, 2015,
...
# Fallback set names if the directory index can't be listed.
DEFAULT_DATASETS = [
...
LIBRARY_FORMAT = "rruff-raman-library"
LIBRARY_VERSION = 1
    ...
    with h5py.File(out_path, 'w') as h5:
        ...
        for ds in datasets:
            ...
            for m in members:
                ...
                g.attrs['quality'] = ds
                g.attrs['orientation'] = orientation or meta.get('ORIENTATION', '')
                g.attrs['wavelength'] = wavelength or meta.get('LASER', '')
...
def main():
    ...
    ap.add_argument("--out", default="rruff_raman_library.h5", help="Output .h5 path")
    ap.add_argument("--datasets", default="", help="Comma-separated set names (default: auto-discover / all)")
    ap.add_argument("--cache-dir", default="rruff_cache", help="Where to cache downloaded zips")
