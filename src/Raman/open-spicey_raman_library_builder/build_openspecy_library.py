...
--------
  ...
  # Baseline-corrected instead, and keep the FT-IR spectra too
  python build_openspecy_library.py --type nobaseline --include-ftir \
...
def build(wav, spectra, meta_rows, args):
    ...
    with h5py.File(args.out, "w") as h5:
        ...
        for n, col in enumerate(names, 1):
            ...
            if not args.include_ftir and stype and "raman" not in stype:
                skipped_type += 1
                continue
            ...
            g.attrs["spectrum_type"] = stype or "raman"
    ...
    if skipped_type:
        print(f"  {skipped_type} skipped as non-Raman (use --include-ftir to keep them)")
    if skipped_filter:
        print(f"  {skipped_filter} skipped by --filter")
    if skipped_bad:
        print(f"  {skipped_bad} skipped as unusable (length mismatch / empty / no bands)")
    if kept == 0:
        print("WARNING: no spectra written. Try --include-ftir, or relax --filter.")
    else:
...
def parse_args(argv=None):
    ...
    sel.add_argument("--include-ftir", dest="include_ftir", action="store_true",
                     help="keep FT-IR spectra as well as Raman")
